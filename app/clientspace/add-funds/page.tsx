...
export default function AddFundsPage() {
  ...
  const [paymentRequestId, setPaymentRequestId] = useState<string>(""); // Store payment request ID for polling
    ...
    const poll = async () => {
      ...
      try {
        const response = await fetch(
          `${API_URL}/api/client/wallet/payment-status/${requestId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  const handleMpesaStkPush = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/client/wallet/add-funds/kopokopo`, {
        method: "POST",
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 font-inter lg:flex">
      ...
      {/* Payment result toast (success or cancelled) */}
      {paymentToast.show && (
