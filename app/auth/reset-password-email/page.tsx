...
export default function ResetPasswordEmailPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch("http://localhost:5000/api/client/auth/reset-password-email", {
        method: "POST",
      ...
      <main className="flex-grow pt-20 bg-gradient-to-br from-orange-50 via-white to-yellow-50 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-6 border border-gray-100">
          ...
          <p className="mt-6 text-center text-sm text-gray-500">
            <Link href="/auth/job-posting" className="text-orange-600 hover:underline">
              Back to login
            </Link>
          </p>
        </div>
